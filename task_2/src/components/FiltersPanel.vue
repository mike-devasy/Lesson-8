<template>
    <aside class="filters-container">
        ...
        <select v-model="filtersObj.make">
            <option v-for="brand in getBrandsList" :key="brand" :value="brand">{{ brand }}</option>
        </select>
        <h4 class="filters-container__title">Модель</h4>
        <select v-model="filtersObj.model">
            <option v-for="model in getModelsList" :key="model" :value="model">{{ model }}</option>
        </select>
...
export default {
    ...
    computed: {
        ...mapGetters(['getCarsList', 'getFilteredCarsList','getBrandsList','getModelsList']),
				carYearList(){
